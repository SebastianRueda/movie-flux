...
data class Result<T>(val status: ResultStatus, val data: T?, val error: String?) {
    companion object {
        fun <T> success(data: T): Result<T> =
            Result(status = ResultStatus.SUCCESS, data = data, error = null)
    ...
    fun setState(onSuccess: (T) -> Unit, onError: ((String) -> Unit)? = null,
